 ...
 * @author davebaol
 */
public class MyAStar<N> implements PathFinder<N> {
    ...
    public MyAStar(IndexedGraph<N> graph) {
        this(graph, false);
    }

    @SuppressWarnings("unchecked")
    public MyAStar(IndexedGraph<N> graph, boolean calculateMetrics) {
        this.graph = graph;
        this.nodeRecords = (NodeRecord<N>[]) new NodeRecord[graph.getNodeCount()];
        this.openList = new BinaryHeap<NodeRecord<N>>();
        if (calculateMetrics) this.metrics = new MyAStar.Metrics();
    }
